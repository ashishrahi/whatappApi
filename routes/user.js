...
const {sendOtpEmail} = require('../services/nodemailer.js')
...
router.post("/signup", async (req, res) => {
 ...
 // Otp Generatin and Saving in Database
const otp = generateOTP(4);
const otpExpiration = new Date(Date.now() + 15 * 60 * 1000);
const otpexpirationstamp = Date.now() + otpExpiration;

console.log(otpexpirationstamp)
sendOtpEmail(email, otp)
//insert otp into database
await query(`UPDATE user SET otp =?, otpexpirationstamp = ? WHERE email =?`, [otp, otpexpirationstamp, email]);
